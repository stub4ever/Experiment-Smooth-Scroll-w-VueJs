...
export default {
    ...
    */
    ...
    addClass (el, className) {
      ...
      if (el.classList) { el.classList.add(classList[0]) } else if (!this.hasClass(el, classList[0])) { el.className += ' ' + classList[0] }
      if (classList.length > 1) { this.addClass(el, classList.slice(1).join(' ')) }
    },
  
    removeClass (el, className) {
      const classList = className.split(' ')
      if (el.classList) { el.classList.remove(classList[0]) } else if (this.hasClass(el, classList[0])) {
    ...
    toggleClass (el, className, bool) {
      if (bool) { this.addClass(el, className) } else { this.removeClass(el, className) }
    },
